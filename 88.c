...
int main(void){
	...
	commend = strtok(fileName, cut);
	realName = strtok(NULL, cut);

	if(strcmp(commend, "ls")==0){
		ls_m(realName);
	}
	else if(strcmp(commend, "mkdir")==0){
		mkdir_m(realName);
	}

	else if(strcmp(commend,"./")==0){
		exec_m(realName);
	}

	else if(strcmp(commend,"vi") == 0){
		vi_m(realName);
	}
